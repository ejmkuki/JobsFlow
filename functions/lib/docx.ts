// Server-side .docx text extraction (Cloudflare Workers runtime). Identical
// logic to src/lib/docx.ts (browser) — kept as a separate copy because
// functions/ and src/ are different TypeScript project roots. A .docx file
// is a ZIP archive; the document body lives at word/document.xml as
// WordprocessingML. We parse just enough of the ZIP central directory to
// locate that one entry, inflate it if needed, and pull text out of <w:t>
// runs. No dependency — ZIP parsing + DecompressionStream('deflate-raw')
// are both native to the Workers runtime.
